...
func runWM(cfg *Config) {
	fmt.Println("Starting pigOS Display Manager...")

	runDMScript := "/home/User/vibecoding-comp/pigOS/vdm/run-dm.sh"

	if _, err := os.Stat(runDMScript); err != nil {
		runDMScript = "./vdm/run-dm.sh"
	}
	if _, err := os.Stat(runDMScript); err != nil {
		runDMScript = "vdm/run-dm.sh"
	}
	if _, err := os.Stat(runDMScript); err != nil {
		fmt.Fprintf(os.Stderr, "wm: Display Manager script not found\n")
		lastExitCode = 1
		return
	}

	fmt.Println("Running:", runDMScript)

	cmd := exec.Command("bash", runDMScript)
	cmd.Stdin = os.Stdin
	...
	err := cmd.Run()
	if err != nil {
		fmt.Fprintf(os.Stderr, "wm: Error: %s\n", err)
		lastExitCode = 1
		return
	}

	fmt.Println("Display Manager exited")
	lastExitCode = 0
}
