...
func (e *LineEditor) handleTabCompletion(prompt string, prevBufW int) int {
	...
	if lastSpace < 0 {
		partial = text
		cmd = text
	} else if lastSpace == len(text)-1 {
		partial = ""
		cmd = strings.TrimSpace(text[:lastSpace])
	} else {
		cmd = strings.TrimSpace(text[:lastSpace])
	...
	if lastSpace < 0 || (lastSpace == len(text)-1) {
		if fn, ok := completionScripts[cmd]; ok {
			candidates = fn(partial)
		}
		if len(candidates) == 0 {
		...
		if fn, ok := completionScripts[cmd]; ok {
			candidates = append(candidates, fn(partial)...)
		}
	}
	...
	if partial == "" {
		info, _ := os.Stat(e.resolvePartialPath(completion))
		...
		if info, _ := os.Stat(e.resolvePartialPath(completion)); info != nil && info.IsDir() {
			completion += "/"
		} else if !strings.HasSuffix(completion, "/") {
			completion += " "
		}
	}
	...
	textBefore := text
	if lastSpace >= 0 {
		textBefore = text[:lastSpace+1]
	}
	e.buf = []rune(textBefore + completion)
	e.cursor = len(e.buf)
	return e.redraw(prompt, prevBufW)
}
